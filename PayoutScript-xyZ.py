...
FEE_PAYOUT_ADDRESS = Web3.toChecksumAddress(
    "0xa0caa7803205026ec08818664c4211aff7565f56")

# Data types
Transaction = namedtuple("Transaction", "from_address to_address amount")
Payout = namedtuple(
    "Payout", "name private_key slp_balance account_address nonce scholar_transaction academy_transaction fee_transaction")
SlpClaim = namedtuple(
    "SlpClaim", "name address private_key slp_claimed_balance slp_unclaimed_balance state")
...
while (len(slp_claims) > 0):
    if (input() == "y"):
        for slp_claim in slp_claims:
            ...
            try:
                slp_utils.execute_slp_claim(
                    slp_claim, nonces[slp_claim.address])
        ...
        for slp_claim in slp_claims:
            if (slp_claim.state["signature"] != None):
                try:
                    slp_total_balance = slp_utils.get_claimed_slp(
                        slp_claim.address)
                except Exception as e:
...
for scholar in accounts["Scholars"]:
    ...
    scholar_payout_amount = math.ceil(
        slp_balance_minus_fees * scholar_payout_percentage)
    ...
    assert(slp_balance == scholar_payout_amount +
           academy_payout_amount + fee_payout_amount)
    ...
    payouts.append(Payout(
        ...
        scholar_transaction=Transaction(
            from_address=account_address, to_address=scholar_payout_address, amount=scholar_payout_amount),
        academy_transaction=Transaction(
            from_address=account_address, to_address=academy_payout_address, amount=academy_payout_amount),
...
while (len(payouts) > 0):
    ...
    for payout in payouts:
        ...
        if (nonces[payout.account_address] <= payout.nonce + 2):
            ...
            try:
                hash = slp_utils.transfer_slp(
                    payout.fee_transaction, payout.private_key, payout.nonce + 2)
                log("DONE")
                log(f"   Hash: {hash} - Explorer: https://explorer.roninchain.com/tx/{str(hash)}")
            except Exception as e:
                log(f"WARNING: " + str(e))
            ...
            # We should never get here because it means the full payout has succeeded and no need for a retry.
            assert(False)
    ...
    for payout in payouts:
        ...
        actual_nonce = nonces[payout.account_address] = web3.eth.get_transaction_count(
