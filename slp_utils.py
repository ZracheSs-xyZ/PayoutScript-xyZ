...
with open('slp_abi.json') as f:
    slp_abi = json.load(f)
slp_contract = web3.eth.contract(address=Web3.toChecksumAddress("0xa8754b9fa15fc18bb59458815510e40a12cd2014"),
                                 abi=slp_abi)
slp_contract_2 = web3_2.eth.contract(address=Web3.toChecksumAddress("0xa8754b9fa15fc18bb59458815510e40a12cd2014"),
                                     abi=slp_abi)
...
def get_unclaimed_slp(address):
    response = requests.get(f"https://game-api.axie.technology/slp/{address}", headers=headers,
                            data="")
...
def execute_slp_claim(claim, nonce):
    ...
    claim_txn = slp_contract.functions.checkpoint(claim.address, claim.state["amount"], claim.state["timestamp"],
